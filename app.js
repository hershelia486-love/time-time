...
const tools = [
  ...
  [2, "목표 만들기", "결과가 보이는 단위로", "지금 시작하면 미래의 삶을 앞당길 프로젝트는 무엇인가요?", "프로젝트 완료 기준"],
...
function weekdayChecklist(selected = [], prefix = "routine") { const selectedDays = new Set(selected); return `<fieldset class="weekday-picker"><legend>고정 실행 요일 <small>여러 날 선택 가능</small></legend><div class="wd-presets"><button type="button" class="wd-preset" data-preset="all" data-prefix="${prefix}">매일</button><button type="button" class="wd-preset" data-preset="weekday" data-prefix="${prefix}">평일</button><button type="button" class="wd-preset" data-preset="weekend" data-prefix="${prefix}">주말</button><button type="button" class="wd-preset" data-preset="none" data-prefix="${prefix}">전체 해제</button></div><div>${["월", "화", "수", "목", "금", "토", "일"].map((label, index) => `<label class="weekday-check"><input type="checkbox" data-${prefix}-weekday value="${index}" ${selectedDays.has(index) ? "checked" : ""}><span>${label}</span></label>`).join("")}</div></fieldset>`; }
...
function todayProjectCard(project, nextAction) {
  if (!project) return `<section class="board-link-card project-board"><div><span class="board-kicker">프로젝트</span><strong>아직 연결한 프로젝트가 없습니다.</strong><small>결과를 하나 정하면 오늘의 다음 행동을 연결할 수 있습니다.</small></div><button class="outline" data-tab="projects">목표 만들기</button></section>`;
...
function redFocus() {
  ...
  return `<section class="card block-red"><div class="block-kick"><span class="cdot red"></span>오늘 꼭 하나 <small>가장 중요한 한 가지</small></div>${reds.length ? reds.map((item, index) => { const area = areas[item.area] || areas.personal; return `<div class="red-focus ${index === 0 ? "first" : ""}"><div class="rf-top">${item.projectId ? `<span class="rf-goal">🎯 ${esc((state.projects.find(p => Number(p.id) === Number(item.projectId)) || {}).title || "목표")}</span>` : ""}<span class="status-tag status-${item.status}">${statusLabels[item.status]}</span></div><div class="rf-title">${esc(item.title)}</div><div class="rf-btns"><button class="outline mini" data-action="postpone-red" data-id="${item.id}">미루기</button><button class="primary mini" data-action="complete-item" data-id="${item.id}">완료 ✓</button></div></div>`; }).join("") : `<p class="fold-empty">오늘 꼭 할 하나가 아직 없어요. 캘린더에서 가장 중요한 일을 정해보세요.</p>`}<button class="linkbtn" data-action="open-calendar">＋ 오늘의 하나 정하기 ›</button></section>`;
}
function blueHabits() {
...
function act(action, id) {
  ...
  if (action === "goal-to-red") { const pid = Number(id); const existing = state.items.find(i => Number(i.projectId) === pid && i.date === todayKey && trackOf(i) === "red" && i.status !== "completed"); if (existing) { state.active = "today"; render(); return; } const title = prompt("오늘 이 목표로 어떤 한 걸음을 할까요?", ""); if (!title || !title.trim()) return; state.items.unshift({ id: Date.now(), title: title.trim(), type: "task", area: "personal", date: todayKey, status: "planned", priority: "high", track: "red", projectId: pid }); logEvent("red", `🔴 ${title.trim()}`, todayKey); state.active = "today"; persist(); render(); return; }
  ...
  if (action === "edit-routine") { state.editingRoutineId = Number(id); render(); const el = document.querySelector("#routine-edit-anchor"); if (el) el.scrollIntoView({ behavior: "smooth", block: "center" }); return; }
